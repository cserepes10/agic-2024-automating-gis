    ...
    for a in range(len(item_list)):
        ...
        for b in range(len(web_items)):
            try:
                ...
                with open (r'D:\COG_ADMIN\MISC_PROJECTS\agic-2024-automating-gis\scripts\json-backups'+ '\\' + folder_list[a] + '\\'+ web_item_name.title+'.json'.replace('\\\\','\\'), "w") as file_handle: #Edit the folder location
...
def hosted_data_backup(creds):
    ...
    layer_item_ids = []
    max = 9999
    hosted_data_folder = r'D:\COG_ADMIN\MISC_PROJECTS\agic-2024-automating-gis\scripts\hosted-data-folder'
